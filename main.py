...
# [기본원칙] S26 Ultra 수직 정렬 대응 및 튕김 방지
    ...
    def __init__(self, **kwargs):
        ...
        for i, group in enumerate(stats):
            for label, val in group:
                container.add_widget(CustomTextInput(hint_text=label))
            if i < len(stats) - 1:
...
class InventoryScreen(BaseScreen):
    def __init__(self, **kwargs):
        ...
        for i in range(3):
            ...
            row.add_widget(Button(text=f"아이템 {i+1} (클릭 시 수정)"))
            row.add_widget(Button(text="저장", size_hint_x=0.2, background_color=get_color_from_hex('#27ae60')))
            row.add_widget(Button(text="삭제", size_hint_x=0.2, background_color=get_color_from_hex('#c0392b')))
...
class PhotoScreen(BaseScreen):
    def __init__(self, **kwargs):
        ...
        for t in ["사진 선택 (다중)", "권한 허용", "업로드", "다운로드"]:
            btn_layout.add_widget(Button(text=t))
...
class PT1App(App):
    def build(self):
        sm = ScreenManager()
        for name in ['account', 'char_select', 'char_info', 'equip_screen', 'inv_screen', 'photo_screen']:
            if name == 'account': sm.add_widget(AccountScreen(name=name))
            elif name == 'char_select': sm.add_widget(CharSelectScreen(name=name))
            elif name == 'char_info': sm.add_widget(CharInfoScreen(name=name))
            elif name == 'equip_screen': sm.add_widget(EquipScreen(name=name))
            elif name == 'inv_screen': sm.add_widget(InventoryScreen(name=name))
            elif name == 'photo_screen': sm.add_widget(PhotoScreen(name=name))
        return sm
