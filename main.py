...
class CustomTextInput(TextInput):
    ...
    def _update_padding(self, *args):
        # 48번 로직 기반: line_height 미확정 시 튕김 방지 처리
...
class BaseScreen(Screen):
    def __init__(self, **kwargs):
        ...
        with self.canvas.before:
            # bg.png 부재 시 검정 배경으로 자동 대체하여 튕김 방지
            Color(0, 0, 0, 1)
...
class PhotoScreen(BaseScreen):
    def __init__(self, **kwargs):
        ...
        for text in ["사진 선택 (다중)", "권한 허용", "업로드", "다운로드"]:
            btn_layout.add_widget(Button(text=text))
...
class PT1App(App):
    def build(self):
        sm = ScreenManager()
        screens = [
            AccountScreen(name='account'), CharSelectScreen(name='char_select'),
            CharInfoScreen(name='char_info'), EquipScreen(name='equip_screen'),
            InventoryScreen(name='inv_screen'), PhotoScreen(name='photo_screen')
        ]
        for s in screens:
            sm.add_widget(s)
        return sm
