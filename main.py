...
<CharSelectScreen>:
    ...
    MDBoxLayout:
        ...
        MDGridLayout:
            cols: 2: padding: "20dp": spacing: "20dp"
            id: char_slots
...
<EquipmentScreen>:
    ...
    MDBoxLayout:
        ...
        ScrollView:
            MDBoxLayout:
                orientation: "vertical": padding: "10dp": spacing: "5dp"
                id: equip_list
...
<PhotoSelectScreen>:
    ...
    MDBoxLayout:
        ...
        MDTopAppBar:
            ...
            left_action_items: [["arrow-left", lambda x: root.go_back()]]
        MDBoxLayout:
            orientation: "vertical": padding: "20dp": spacing: "20dp"
            MDLabel:
                text: "핸드폰 권한 허용 및 사진 다중 선택"
                halign: "center"
            MDRaisedButton:
                text: "사진 선택 및 업로드"
                pos_hint: {"center_x": .5}
...
class MainScreen(MDScreen):
    def on_enter(self):
        ...
        for i in range(3):
            self.ids.account_list.add_widget(OneLineListItem(text=f"Account_ID_{i}", on_release=lambda x: self.go_char()))
    def go_char(self): self.manager.current = "char_select"

class CharSelectScreen(MDScreen):
    def on_enter(self):
        self.ids.char_slots.clear_widgets()
        for i in range(1, 7): # [제1원칙] 6개 슬롯 보존
            self.ids.char_slots.add_widget(MDRaisedButton(text=f"케릭 {i}", on_release=lambda x: self.go_info()))
...
class InventoryScreen(MDScreen):
    def on_enter(self):
        ...
        for i in range(5):
            item = TwoLineAvatarIconListItem(text=f"아이템 {i}", secondary_text="클릭하여 수정")
            item.add_widget(IconLeftWidget(icon="content-save", on_release=lambda x: print("저장")))
            item.on_release = self.go_photo # 다음 창 이동
