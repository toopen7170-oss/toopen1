...
class DiagnosticLabel(Label):
    """자가 진단 전용: 폰트가 없어도 시스템 폰트로 무조건 출력"""
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        if HAS_FONT:
            self.font_name = "CustomFont"
        self.font_size = '16sp'
        self.color = (1, 0, 0, 1)  # 경고용 빨간색

class CustomTextInput(TextInput):
    """S26 Ultra 최적화: 수직 정렬 및 더블 터치 전체보기"""
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def show_full_text_popup(self):
        ...
        close_btn = Button(text="확인", size_hint_y=0.2, background_color=get_color_from_hex('#2980b9'))
        if HAS_FONT: close_btn.font_name = "CustomFont"
        content.add_widget(full_input); content.add_widget(close_btn)
        popup = Popup(title="전체 내용 수정", content=content, size_hint=(0.9, 0.6))
    ...
    def update_list(self, *args):
        ...
        for acc in self.accounts.keys():
            if query in acc.lower():
                ...
                del_btn.bind(on_release=lambda x, n=acc: show_confirm("삭제", f"'{n}'을 삭제할까요?", lambda: self.delete_acc(n)))
                row.add_widget(btn); row.add_widget(del_btn); self.acc_list.add_widget(row)

    def add_acc_pop(self, *args):
        content = BoxLayout(orientation='vertical', padding=15, spacing=15)
        new_id = CustomTextInput(hint_text="계정 ID")
...
class SlotScreen(BaseScreen):
    ...
    def on_enter(self):
        ...
        for i in range(1, 7):
            ...
            btn = Button(text=char_name, background_color=(0.2, 0.2, 0.5, 0.6))
            if HAS_FONT: btn.font_name = "CustomFont"
...
class DetailScreen(BaseScreen):
    ...
    def __init__(self, **kwargs):
        ...
        for idx, group in enumerate(groups):
            for s in group:
                ...
                row.add_widget(lbl); row.add_widget(ti); self.grid.add_widget(row)
            if idx < len(groups)-1: self.grid.add_widget(BoxLayout(size_hint_y=None, height=40))
        self.scroll.add_widget(self.grid); self.main_layout.add_widget(self.scroll)
        ...
        if HAS_FONT: back.font_name = "CustomFont"
        self.main_layout.add_widget(back); self.add_widget(self.main_layout)
    ...
    def save_data(self, *args):
        ...
        show_confirm("저장", f"'{name}' 저장되었습니다.", lambda: None)
...
class EquipScreen(BaseScreen):
    def __init__(self, **kwargs):
        ...
        for i in ["한손무기", "두손무기", "갑옷", "방패", "장갑", "부츠", "링1", "링2", "아뮬렛"]:
...
class InvenScreen(BaseScreen):
    def __init__(self, **kwargs):
        ...
        add.bind(on_release=self.add_item); layout.add_widget(add)
    ...
    def add_item(self, *args):
        ...
        del_btn = Button(text="X", size_hint_x=0.2, background_color=get_color_from_hex('#c0392b'))
        del_btn.bind(on_release=lambda x: show_confirm("삭제", "삭제할까요?", lambda: self.list.remove_widget(row)))
...
class PristonTaleApp(App):
    def build(self):
        try:
            ...
            # [자가 진단] 치명적 오류 시 화면에 강제 출력
            err_box = BoxLayout(orientation='vertical', padding=50)
            err_box.add_widget(DiagnosticLabel(text="[치명적 오류 발생]\n\n" + str(e) + "\n\n" + traceback.format_exc()))
            return err_box
